...
public class XMLNodesIteratorTest {
   ...
   public void testParse() {
      System.out.println("XMLNodesIteratorTest : testParse");
   ...
   public void testCreateIterator() {
      System.out.println("XMLNodesIteratorTest : testCreateIterator");
   ...
   public void testIterate() {
      System.out.println("XMLNodesIteratorTest : testIterate");
      assertNotNull("iterator", iterator);
      ...
      assertEquals("Node name", "first", name);
      XMLNode parent = node.getParent();
      assertTrue("must be the root", root == parent);

      assertTrue("hasNext", iterator.hasNext());
      XMLNode node2 = iterator.next();
      assertNotNull("node", node2);
      assertTrue("must have a name attribute", node2.hasAttribute("name"));
      name = node2.getAttributeValue("name");
      assertEquals("Node name", "second", name);
      parent = node2.getParent();
      assertTrue("must be the first node", node == parent);

      assertTrue("hasNext", iterator.hasNext());
      XMLNode node3 = iterator.next();
      assertNotNull("node", node3);
      assertTrue("must have a name attribute", node2.hasAttribute("name"));
      name = node3.getAttributeValue("name");
      assertEquals("Node name", "third", name);
      parent = node2.getParent();
      assertTrue("must be the first node", node == parent);
