/*------------------------------------------------------------------------------
 * Copyright (C) 2006, 201, 2012, 2016, 2017 Herve Girod
 *
...
@Category(cat = "geom")
public class ClippableShapeTest {
   private static final double DELTA = 0.5f;

   public ClippableShapeTest() {
   }
   ...
   public void testNoIntersection() {
      ...
      List<Shape> v = norm.convertPath(out);
      assertEquals("Should have one Shape", 1, v.size());
      assertTrue("Shape must be an Arc2D", v.get(0) instanceof Arc2D);

      Arc2D arc1 = (Arc2D) (v.get(0));
    ...
    */
   @Test
   public void testIntersectionHalfCircle2() {
      System.out.println("ClippableShapeTest : testIntersectionHalfCircle2");
      Arc2D.Float arc = new Arc2D.Float(-50, -50, 100f, 100f, 90f, 360f, Arc2D.OPEN);
      Rectangle2D.Float clip = new Rectangle2D.Float(-100f, 0f, 200f, 100f);
      ClippableShape in = new ClippableShape(arc);
      in.intersect(new Area(clip));
      Shape out = in.getPeerShape();
      ShapeNormalizer norm = new ShapeNormalizer();
      ...
      Arc2D arc1 = (Arc2D) (v.get(0));
      assertEquals(arc1.getAngleStart(), -180d, DELTA);
      assertEquals(arc1.getAngleExtent(), 180d, DELTA);
   }

   /**
    * Test of intersect method. The Shape is a circle, and the clip takes only a quarter of it
    */
   @Test
   public void testIntersectionQuarterCircle() {
      System.out.println("ClippableShapeTest : testIntersectionQuarterCircle");
      Arc2D.Float arc = new Arc2D.Float(-50, -50, 100f, 100f, 90f, 360f, Arc2D.OPEN);
      Rectangle2D.Float clip = new Rectangle2D.Float(-100f, -100f, 100f, 100f);
      ClippableShape in = new ClippableShape(arc);
      in.intersect(new Area(clip));
      Shape out = in.getPeerShape();
      ShapeNormalizer norm = new ShapeNormalizer();
   ...
   /**
    * Test of intersect method. The Shape is a Line.
    */
   @Test
   public void testIntersectionLine() {
      System.out.println("ClippableShapeTest : testIntersectionLine");
      Line2D.Float arc = new Line2D.Float(0, 50, 100, 50);
      Rectangle2D.Float clip = new Rectangle2D.Float(0, 0, 25, 100);
      ClippableShape in = new ClippableShape(arc);
      in.intersect(new Area(clip));
      Shape out = in.getPeerShape();
      ShapeNormalizer norm = new ShapeNormalizer();
      List<Shape> v = norm.convertPath(out);
      assertEquals("Should have one Shape", 1, v.size());
      assertTrue("Shape must be an Line2D", v.get(0) instanceof Line2D);

      Line2D line = (Line2D) v.get(0);
      assertEquals("Line coordinates", 0d, line.getX1(), DELTA);
      assertEquals("Line coordinates", 50d, line.getY1(), DELTA);
      assertEquals("Line coordinates", 25d, line.getX2(), DELTA);
      assertEquals("Line coordinates", 50d, line.getY2(), DELTA);
   }
}
